...
public class PageDTO {
  ...
  private MeetingMemberCriteria mcri;
  ...
  public PageDTO(MeetingMemberCriteria mcri, int total) {
	    ...
	    // 현재 페이지 번호를 가지고 endPage 계산 (페이지 네비게이션에서 나올 끝 번호)
	    // ( (12/10.0) = 2 ) * 10 = 20 (endPage=>페이지 네비게이션이 11, 12, 13,...20까지 나와야 된다)
	    this.endPage = (int) (Math.ceil(mcri.getMemberpageNum() / 10.0)) * 10;
	    ...
	    // 31,32,33..40
	    // 41,42,43, 44, 45 (45가 마지막 번호이다.)
	    int realEnd = (int) (Math.ceil((total * 1.0) / mcri.getMemberamount()));
